...
define('FILE_VERSION', 'V.0.0.7 (Fix Inserimento Parametri)');
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Recupero campi aggiuntivi dallo schema DB
    $prop = $_POST['prop'] ?? '';
    $creg = $_POST['creg'] ?? '';
    $treg = $_POST['treg'] ?? '';
    ...
    try {
        if (isset($_POST["inserisci"])) {
            // SQL con 10 parametri come da schema
            $sql = "INSERT INTO AP_Arnie (AR_CODICE, AR_NOME, AR_LUOGO, AR_PROP, AR_CREG, AR_TREG, AR_ATTI, AR_DATA, AR_NUCL, AR_Note) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($sql);
            // Bind di 10 variabili corrispondenti ai 10 punti di domanda
            $stmt->bind_param("ssisssisis", $_POST['codice'], $_POST['nome'], $_POST['luogo_id'], $prop, $creg, $treg, $atti, $data_db, $nucl, $_POST['note']);
            if ($stmt->execute()) {
                header("Location: arnie.php?status=insert_success");
                exit();
            }
            ...
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssisssisisi", $_POST['codice'], $_POST['nome'], $_POST['luogo_id'], $prop, $creg, $treg, $atti, $data_db, $nucl, $_POST['note'], $id_post);
            if ($stmt->execute()) {
                header("Location: arnie.php?status=update_success");
                exit();
            }
            ...
            if ($stmt->execute()) {
                header("Location: arnie.php?status=delete_success");
                exit();
            }
        }
...
if (isset($_GET["status"])) {
    $st = $_GET["status"];
    if ($st == "insert_success") $messaggio = "<p class='successo' style='color:green; font-weight:bold;'>Arnia inserita correttamente!</p>";
    if ($st == "update_success") $messaggio = "<p class='successo' style='color:green; font-weight:bold;'>Arnia aggiornata!</p>";
    if ($st == "delete_success") $messaggio = "<p class='successo' style='color:green; font-weight:bold;'>Arnia eliminata!</p>";
}
